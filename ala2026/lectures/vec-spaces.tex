...
\begin{frame}\FrameTitle{Unit Vector}{Unit 1}
    ...
    \vsp
    $e_i$ is an \emph{n}-vector. $(e_i)_j$ is a number, its \emph{j}-th entry.
\end{frame}
...
\begin{frame}\FrameTitle{Properties of Vector Addition}{Unit 1}
    For any $n$-vectors a, b, and c, the following properties hold:
    \begin{itemize}
        \item Vector addition is \emph{commutative}: a + b= b + a.
        \item Vector addition is \emph{associative}: (a + b) + c = a + (b + c). We can therefore
        write both as a + b + c.
        \item a + \textbf{0} = \textbf{0} + a = a. Adding the zero vector to a vector has no effect.$^{\dagger}$
        \item a - a = \textbf{0}. Subtracting a vector from itself gives the zero vector.
    \end{itemize}
    \vsp
...
\begin{frame}\FrameTitle{Scalar-vector multiplication}{Unit 1}
    For any $a \in \mathbb{R}^n$, and scalars $\alpha, \beta, \gamma \in \mathbb{R}$,
    the following properties hold:
    \begin{itemize}
        \item scalar-vector multiplication is \emph{commutative}: $\alpha a = a \alpha$.
        \item scalar-vector multiplication is \emph{associative}: $(\beta \gamma) a = \beta (\gamma a)$.
        \item 1 a = a. Multiplying a vector by the scalar 1 has no effect.
        \item 0 a = \textbf{0}. Multiplying a vector by the scalar 0 gives the zero vector.
        \item $\beta (a + b) = \beta a + \beta b$. Scalar multiplication distributes over vector addition.
        \item $(\beta + \gamma) a = \beta a + \gamma a$. The distributive property of scalar-vector multiplication.
